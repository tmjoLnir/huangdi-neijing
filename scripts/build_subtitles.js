...
function burnCommand(srtPath, fmt) {
  ...
  return [
    ...
    `  > ${assPath}`,
    "",
    `# 2. Burn. Do NOT rescale the cut to ${fmt.width}x${fmt.height} first: the assembler's`,
    `#    output follows the source clips (a 480p draft delivered 496x864, ~2% wider`,
    `#    than true 9:16), so forcing the nominal size stretches the picture. The style`,
    `#    values above are in PlayRes space and scale with the frame, so the measured`,
    `#    fit holds at the cut's own geometry — ffprobe it rather than assuming.`,
    "ffmpeg -i <cut>.mp4 \\",
